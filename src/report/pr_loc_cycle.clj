...
(defn do-it!
 ...
 ([user repo params]
  (let [prs (github.prs/all-prs! user repo params)
        ...
        loc:totals (map report.pr-loc/datum->loc-total loc:datums)
        loc:diffs (map report.pr-loc/datum->loc-diff loc:datums)
        cycle:datums (map report.pr-cycle-time/pr->datum full-prs)
        cycle:cycle-times (map report.pr-cycle-time/datum->cycle-time cycle:datums)]
